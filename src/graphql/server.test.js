...
describe("graphql server", () => {
  ...
  it("should fail if not auth", async () => {
    await expect(server.executeOperation({ query: "test" })).rejects.toThrow(
      AuthenticationError
    );
  });
  ...
  it("should serve listings", async () => {
    ...
    expect(res?.data?.listings).toEqual(
      dummyListings.listings.map((l) => ({ listingId: l.listingId }))
    );
  });

  it("should serve listings with filtering city", async () => {
    const res = await server.executeOperation(
      { query: GET_LISTINGS_CITY1 },
      { req }
    );
    expect(res?.errors).toBeUndefined();
    expect(res?.data).not.toBeUndefined();
